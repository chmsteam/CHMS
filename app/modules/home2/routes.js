...
function findcontract(req, res, next){
  var db = require('../../lib/database')();
  db.query(`SELECT * FROM tblcontract INNER JOIN tbltransaction ON intTRequestID = intConTransID WHERE intConHWID=? AND strConStatus IN ('Waiting')`,[req.session.user], function (err, results) {
      console.log(''+req.params.userid);
      req.cont= results;
      return next();
  });
